...
### JAX-RS Java API for Restful Web Services
  ...
  - JAX-RS Filters = API constructs that are used to filter requests and responses. There are two kinds of filters, request filters and response filters. A request filter can be used to filter requests through JAX-RS resources, that is intercept the request and do certain things with it. It can optionally be decided whether those requests should hit on the resource methods or denied access. There are two kinds of request filters, pre-matching and post-matching. A pre-match filter is invoked before the runtime matches a request to a resource method. The particular URL will be intercepted even before their request is matched to the given method. A post-match filter is invoked when a URL is matched to  a certain resource method. Response filters are used to filter responses to the client. Example, the response can instruct the client to cache every get method that is invoked in a resource method.
  -JSON-B API = underlying API that converts a given entity to its corresponding JSON object and vice versa. For instance, @JsonbDateFormat annotation can be used to customize the format of a date object. @JsonbTransient annotation makes the Jsonb marshaller skip the serialization of the annotated entity.
